...
def classify(model, data, grained, index_to_word):
    evaluator = Evaluators[grained]()
    keys = evaluator.keys()
    correct = {key: np.array([0]) for key in keys}
    wrong = {key: np.array([0]) for key in keys}
...
if __name__ == '__main__':
    ...
    train_data, dev_data, test_data, classify_data = data.gen_data(args.grained)
	...
	classify(model, classify_data, args.grained, index_to_word)
